...
(7,"metric","Perché la metrica ovvia porta a una conclusione falsa",
 ...
 ("Cosa mostra","<p>Blocco 1, quattro livelli di tracing. A sinistra la colonna <code>run</code> che rt-app riporta nativamente; a destra il <code>budget</code> (<code>run + slack</code>) relativo al livello 0.</p>"),
 ("Come leggerlo","<p>A sinistra il livello 1 misura <strong>1954 µs contro i 1984 del livello 0</strong>: risulterebbe più veloce del codice <em>senza alcun tracing</em>. A destra, la stessa identica campagna dice che i livelli 1 e 2 non sono misurabili e solo il livello 3 costa ~13 µs.</p>"),
 ("Perché è significativo","<p>È una figura sulla <strong>validità delle misure</strong>, non sul sistema sotto test, e giustifica la scelta della variabile di risposta usata in tutte le altre. Un overhead non può rendere il codice più veloce: il risultato di sinistra è un artefatto di <strong>layout del binario</strong> — ogni livello è un eseguibile diverso, e l'allineamento del codice del busy-loop cambia — e vale ~30 µs, cioè <strong>più del segnale da misurare</strong>.</p><p>La colonna <code>run</code> cronometra solo il busy-loop, mentre gli span nascono fuori da quella finestra. Lo stesso vale per <code>period</code> (<code>end − start</code> della stessa riga), che al livello 3 <em>si accorcia</em> di 15-24 µs proprio dove l'overhead cresce. Sommando <code>slack</code> si recupera l'intero tempo dell'iterazione, e l'artefatto si cancella: ciò che <code>run</code> guadagna, <code>slack</code> lo perde esattamente.</p>"),
 ("Implicazione pratica","<p>Chi analizzasse questo DoE con le colonne native di rt-app concluderebbe che la strumentazione OTel <em>migliora</em> le prestazioni.</p>")]),
...
.note ol{{margin:0 0 11px;padding-left:1.25em}} .note li{{margin-bottom:6px}}
